...
import { Heart, CheckCircle, RefreshCcw } from "lucide-react";
...
export default function GivePage() {
  ...
  async function handleGive(e: React.FormEvent) {
    ...
    const loadPaystack = () => {
      if (document.getElementById("paystack-script")) {
        openPaystack(PUBLIC_KEY, reference, amount);
        return;
      }
      const script = document.createElement("script");
      script.id = "paystack-script";
      script.src = "https://js.paystack.co/v1/inline.js";
      script.onload = () => openPaystack(PUBLIC_KEY, reference, amount);
  ...
  function openPaystack(publicKey: string, reference: string, amount: number) {
    ...
    const handler = window.PaystackPop.setup({
      ...
      callback: async (response: { reference: string }) => {
        try {
          ...
          else toast.error("Payment verification failed or is pending. We will confirm via email.");
        } catch (err) {
          setLoading(false);
    ...
    <div className="min-h-screen bg-navy-950">
      <section className="relative py-20 px-6 text-center overflow-hidden">
        ...
        <div className="relative z-10 px-6">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-white mb-4 leading-tight">Kingdom <span className="gradient-text">Giving</span></h1>
      ...
      <div className="max-w-xl mx-auto px-6 pb-24">
        {success ? (
          <div className="glass rounded-3xl p-12 text-center">
            ...
            <p className="text-slate-400 text-lg mb-8">Your giving has been received. May God bless you abundantly.</p>
            <button
              onClick={() => {
                setSuccess(false);
                setForm({ ...form, amount: "" });
              }}
              className="px-6 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl font-semibold transition-colors flex items-center gap-2 mx-auto"
            >
          ...
          <div className="glass rounded-3xl p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
              {CATEGORIES.map((c) => (
                <button
                  ...
                  className={`p-4 rounded-2xl text-left transition-all border ${form.category === c.value ? "gradient-gold text-navy-950 border-gold-400" : "glass text-slate-300 border-white/5 hover:border-gold-500/30"}`}
                >
                  <p className="font-bold text-sm uppercase tracking-wider">{c.label}</p>
                  <p className={`text-xs mt-1.5 leading-relaxed ${form.category === c.value ? "text-navy-900/70" : "text-slate-500"}`}>{c.desc}</p>
                </button>
              ))}
            </div>
            ...
            <form onSubmit={handleGive} className="space-y-5">
              <div>
                ...
                <input required value={form.donorName} onChange={(e) => setForm({ ...form, donorName: e.target.value })} placeholder="John Doe" className="w-full px-4 py-3.5 glass rounded-xl text-white placeholder-slate-500 border border-white/10 focus:outline-none focus:border-gold-500/50 transition-all" />
              </div>
              <div>
                <label className="text-slate-400 text-sm mb-1.5 block ml-1">Email Address *</label>
              ...
              <div>
                <label className="text-slate-400 text-sm mb-1.5 block ml-1">Amount ({currencySymbol}) *</label>
                <input required type="number" min={minAmount} value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder={form.currency === "NGN" ? "5000" : "50"} className="w-full px-4 py-3.5 glass rounded-xl text-white placeholder-slate-500 border border-white/10 focus:outline-none focus:border-gold-500/50 transition-all text-xl font-bold" />
              </div>
